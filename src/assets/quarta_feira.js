function addTask() {
  ...
  if (tasks.name.trim() !== "") {
    ...
    var taskList = JSON.parse(localStorage.getItem("tasks_quarta")) || [];

    // Adiciona a nova tarefa à lista
    taskList.push(tasks);

    // Salva a lista atualizada no localStorage
    localStorage.setItem("tasks_quarta", JSON.stringify(taskList));
...
function displayTasks() {
  ...
  taskList.forEach(function(task, index) {
   ...
   //   cardBody.className = "card-body";
    ...
    completeButton.className = 'btn btn-primary mr-2 float-roght';
